    ...
    for e in range(n_epochs):
        ...
        for batch in tqdm_:
            ...
            loss_fake = torch.mean(
                F.relu(1.0 + pred_fake))  # loss_discriminator(pred_fake, torch.zeros_like(pred_fake))
            ...
            bce = torch.mean(
                -pred_fake)  # loss_discriminator(pred_fake, torch.ones_like(pred_fake)) #propobabilità dei falsi di essere scambiati per veri
