...
def evaluate_model(test_dir_prefix, output_generated, video_prefix, filename, from_second=0, to_second=None,
    ...
    cap_hq = cv2.VideoCapture(str(test_dir_prefix) + f"/originali/{video_prefix}")
    print(str(test_dir_prefix) + f"/originali/{video_prefix}")
    ...
    if output_generated and not skip_model_testing:
        ...
        vmaf_command = f"{ffmpeg_path} -nostats -loglevel 0 \
            -r {fps} -i {dest_dir /  'originali' / (video_prefix)} \
...
if __name__ == '__main__':
    ...
    videos = os.listdir(test_dir / "originali")#[vid.strip(".y4m") for vid in os.listdir(test_dir) if vid.endswith('.y4m') and '1080' in vid]

    second_start = 0
    second_finish = 32  # test no more than the 2nd minutes - none of the test videos last so much
